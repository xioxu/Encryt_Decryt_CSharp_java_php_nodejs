...
namespace demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var publicKey =
                @"<RSAKeyValue><Modulus>18lmL3io599boCL7+8TtTkSYI2b9uLjAfw0t1gndISoLu5w8YmvYLBGUpU/IOwinCrJgRSU69xXtTNXS3K6I2IGdWax+Ts4f7kj1VSXGrjAAyAxUoBMwTry69X9J2bxcHu/dxpMuBLt/Vzh+zRyZWYqCdx/2t8Hbl90ydgA8Urs=</Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";
           ...
           // provider.Encrypt("test");
            ...
            var encrptInJavaWithPubKey =
                "jHj/kLDp3DO5JFNmzMG3L8eREg/W25VQDl8Z+skaER/RQNqvwhe3SFa7VHD64hPBTKhghxoXTbhm0xH/ViZDOVltPRWHBesyE2O6bNnvZ43XVtnGSZ7ub5gHkGp6E0ghngTsycatJDGMyI2VjEoD7RfZC4R+ICo8Dw2fWkNxlA0=";
            var enctrpted = Convert.FromBase64String(encrptInJavaWithPubKey);

            var plaintext = provider.Decrypt(enctrpted,false);
            Console.WriteLine("");
            Console.WriteLine(".net解密java中加密的内容：");
            Console.WriteLine(Encoding.UTF8.GetString(plaintext));

            var netEncrpt = rsaEncrpt( Encoding.UTF8.GetString(plaintext), publicKey);

            plaintext = provider.Decrypt(Convert.FromBase64String(netEncrpt), false);

            Console.WriteLine(".net加密内容：");
            Console.WriteLine(netEncrpt);
            Console.WriteLine(".net解密内容：");
            Console.WriteLine(Encoding.UTF8.GetString(plaintext));

            Console.Read();
        }

        public static string rsaEncrpt(string paintext,string publickKey)
        {
            RSACryptoServiceProvider provider = new RSACryptoServiceProvider();
            provider.FromXmlString(publickKey);
            var messageBytes = Encoding.UTF8.GetBytes(paintext);
            var encryptedMessageBytes = provider.Encrypt(messageBytes, false);
